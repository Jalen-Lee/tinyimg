...
function ToolbarSelect() {
  const {
    ...
    inSaving,
    inCompressing
  } = useCompressionStore(useSelector(['files','selectedFiles','setSelectedFiles','inSaving','inCompressing']));
  ...
  const handleSelectMode: MenuProps['onClick'] = ({ key }) => {
    switch(key){
      ...
      case RowSelection.INVERT:
        setSelectedFiles(files.filter(file => !selectedFiles.includes(file.id)).map(file => file.id));
        break;
      case RowSelection.UNCOMPRESSED:
        setSelectedFiles(files.filter(file => file.compressStatus === IScheduler.TaskStatus.Pending).map(file => file.id));
        break;
      case RowSelection.UNSAVED:
        setSelectedFiles(files.filter(file => file.compressStatus === IScheduler.TaskStatus.Completed).map(file => file.id));
        break;
    }
  }
